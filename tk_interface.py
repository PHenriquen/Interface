...
class AppSerial:
    ...
    def build_ui(self):
        ...
        self.temp_label = tk.Label(left, text="-- C", font=("Segoe UI", 30, "bold"))
        self.temp_label.grid(row=0, column=0, sticky="w")

        self.sensor_label = tk.Label(left, text="Sensor: --")
        self.sensor_label.grid(row=1, column=0, sticky="w")

        self.quality_label = tk.Label(left, text="Qualidade: --")
        self.quality_label.grid(row=2, column=0, sticky="w")

        self.status_label = tk.Label(left, text="Status: aguardando")
        self.status_label.grid(row=3, column=0, sticky="w", pady=(0, 6))

        self.last_line = tk.Label(left, text="Ultima linha: --")
        self.last_line.grid(row=4, column=0, sticky="w", pady=(0, 6))

        actions = tk.Frame(left)
        actions.grid(row=5, column=0, sticky="ew", pady=(0, 6))
        actions.grid_columnconfigure(0, weight=1)
        actions.grid_columnconfigure(1, weight=1)

        ttk.Button(actions, text="Limpar", command=self.clear_history).grid(
            row=0, column=0, sticky="ew", padx=(0, 4)
        )
        ttk.Button(actions, text="Atualizar portas", command=self.refresh_ports).grid(
            row=0, column=1, sticky="ew", padx=(4, 0)
        )

        self.listbox = tk.Listbox(left, font=("Consolas", 10), height=10)
        ...
        self.connect_btn = ttk.Button(sbtn, text="Conectar", command=lambda: self.start("serial"))
        self.connect_btn.grid(row=0, column=0, sticky="ew", padx=(0, 4))
        ttk.Button(sbtn, text="Desconectar", command=self.disconnect).grid(
            row=0, column=1, sticky="ew", padx=(4, 0)
        )
        ...
        self.sim_interval = ttk.Combobox(
            sim, state="readonly", values=["0.2", "0.5", "1.0", "2.0"], width=10
        )
        self.sim_interval.set("1.0")
        self.sim_interval.grid(row=1, column=0, sticky="w", pady=(0, 6))

        sim_btn = tk.Frame(sim)
        sim_btn.grid(row=2, column=0, sticky="ew")
        sim_btn.grid_columnconfigure(0, weight=1)
        sim_btn.grid_columnconfigure(1, weight=1)

        ttk.Button(sim_btn, text="Iniciar", command=lambda: self.start("sim")).grid(
            row=0, column=0, sticky="ew", padx=(0, 4)
        )
        ttk.Button(sim_btn, text="Parar", command=self.disconnect).grid(
            row=0, column=1, sticky="ew", padx=(4, 0)
        )
    ...
    def refresh_ports(self):
        if not HAS_PYSERIAL:
            ...
            self.port_combo.set("")
            self.detect_label.configure(text="Arduino: pyserial nao instalado")
            return

        usb = [p.device for p in list_serial_ports()]
        arduino = detect_arduino_ports()
        ports = arduino or usb
        ...
        if arduino:
            txt = f"Arduino: {', '.join(arduino)}"
        elif usb:
            txt = "Arduino: nenhum identificado (usando USB encontrado)"
        else:
            txt = "Arduino: nenhum dispositivo USB"
    ...
    def start(self, mode):
        ...
        if mode == "serial":
            if not HAS_PYSERIAL:
                self.status_label.configure(text="Status: pyserial nao instalado")
                return
    ...
    def clear_history(self):
        ...
        self.errors = 0
        self.listbox.delete(0, tk.END)
        self.last_line.configure(text="Ultima linha: --")
